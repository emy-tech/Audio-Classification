...
Mun ID: 201794630
Purpose: Decision Tree MultiClass Classification
...
#Loading of DataSet
metadata = pd.read_csv('New_sound/NewSound.csv',
...
# Iteration through each sound file and extract the MFCC feature 
features = []
...
target_names = ['Atlantic spotted dolphin', 'Long Finned Pilot Whale' ,'North Right Whale',   'South Right Whale', 'Harp-seal', 'Narwhal']
print(classification_report(y_test.argmax(axis=1), y_pred.argmax(axis=1), target_names=target_names))

#Definition if parameters for GridSearchCV
param = {'criterion':['gini','entropy'],
            'max_features':['auto', 'log2', 'sqrt'],
            'max_depth':[1,2,3,4,5,10,15,20,40,60,80,100],
            'max_leaf_nodes':[2,3,4,5,10,15,20,40,60,80,100]
}
...
#GridSearchCV
grid = GridSearchCV(estimator=DTC, param_grid=param, cv=KF, scoring="accuracy")
